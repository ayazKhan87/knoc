(() => {
	...
	function showPreviousGameResults(previousGame) {
			...
			<div class="error-panel">
				...
				${hasReward ? `
					<div style="margin-top: 20px; padding: 15px; background: rgba(0,255,255,0.1); border-radius: 8px;">
						...
						<input type="text" id="claimQrInputPrevious" placeholder="Enter claim QR code" style="width: 100%; padding: 8px; margin: 10px 0; border: 1px solid #ccc; border-radius: 4px; background: #222; color: #fff;">
						<input type="text" id="passKeyInputPrevious" placeholder="Enter 4-digit pass key" maxlength="4" style="width: 100%; padding: 8px; margin: 10px 0; border: 1px solid #ccc; border-radius: 4px; background: #222; color: #fff;">
				...
				<button onclick="location.reload()" class="btn">Close</button>
			</div>
		`;
		...
		if (hasReward) {
			...
			const passKeyInput = document.getElementById('passKeyInputPrevious');
